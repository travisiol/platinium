  ...
  if (isConnected && address) {
    if (wrongNetwork) {
      return (
        <button
          ...
          className={clsx(
            "rounded-full border border-plat-500/50 bg-plat-500/10 px-4 py-2 text-xs font-semibold uppercase tracking-wider text-plat-300 transition hover:bg-plat-500/20 disabled:opacity-60",
            className,
          )}
        >
    ...
    return (
      <button
        ...
        onClick={() => disconnect()}
        className={clsx(
          "group flex items-center gap-2 rounded-full border border-line-strong bg-white/5 px-4 py-2 text-xs font-medium text-ice transition hover:border-plat-500/50",
          className,
        )}
  ...
  return (
    <div className={clsx("flex flex-col items-end gap-1", className)}>
      <button
        ...
        className="rounded-full bg-plat-400 px-4 py-2 text-xs font-semibold uppercase tracking-wider text-black transition hover:bg-plat-300 disabled:cursor-not-allowed disabled:opacity-50"
      >
        {isConnecting
          ? "Connecting…"
          : injectedConnector
